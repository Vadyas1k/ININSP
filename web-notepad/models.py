...
def update_note(session_id, note_id, title=None, content=None):
    ...
    if title is not None:
        conn.execute(
            "UPDATE notes SET title = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ? AND session_id = ?",
            (title, note_id, session_id),
        )
    if content is not None:
        conn.execute(
            "UPDATE notes SET content = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ? AND session_id = ?",
            (content, note_id, session_id),
        )
    conn.commit()
    conn.close()
...
def delete_note(session_id, note_id):
    conn = get_connection()
    conn.execute(
        "DELETE FROM notes WHERE id = ? AND session_id = ?", (note_id, session_id)
    )
    conn.commit()
    conn.close()
